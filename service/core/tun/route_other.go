...
// SetupTunRouteRules is retained for the Linux build tag symmetry but is a no-op here.
// On Windows the default route (172.19.0.2) is added by the TunPostStartScript; cleanup
// is still handled by CleanupTunRouteRules so the route is removed when TUN stops.
